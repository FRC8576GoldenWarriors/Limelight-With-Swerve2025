...
public class RobotContainer {
  ...
  // public static final AprilTagStats m_AprilTag = new
  // AprilTagStats(Constants.VisionConstants.nameConstants.cameraName,
  // Constants.VisionConstants.nameConstants.publishName,Constants.VisionConstants.nameConstants.tabName);
  // //public static final PoseEstimator m_poseEstimator = new PoseEstimator();
  // public final LEDStrip ledStrip;
  // public static final LED m_led = new LED(Constants.LEDConstants.LED_PORT1,
  // Constants.LEDConstants.LedLength1);
  ...
  private void configureBindings() {
    ...
    driverController
        .leftBumper()
        .and(() -> aprilTagStatsLimelight.hasValidTargets() && aprilTagStatsLimelight.getID() == 14)
        .whileTrue(AlignToAprilTag);
    ...
    // operatorController.a().whileTrue(new IntakeDown(m_Intake));//new SequentialCommandGroup(new
    // IntakeIn(m_Intake), new IntakeUp(m_Intake), new Shoot(m_Shooter)));
    ...
    // operatorController.y().and(operatorController.leftBumper()).whileTrue(new
    // ShooterUp(m_Shooter));
    // operatorController.b().and(operatorController.leftBumper()).whileTrue(new
    // ShooterDown(m_Shooter));
    ...
    // //operatorController.y().onTrue(new Shoot(m_ShooterRoller)); //b button ends shoot command,
    // defined in shoot command
    // //Shintake
    ...
    // //
    // .and(()->Constants.VisionConstants.distanceConstants.useableIDs.contains(m_AprilTag.getID()))
    // // .onTrue(new SequentialCommandGroup(new ParallelCommandGroup(new AdjustRobotPos(drivetrain,
    // m_AprilTag),new SetShooterAngle(m_Shooter,Constants.ShooterConstants.kShooterAutoAngle)),new
    // Shoot(m_ShooterRoller)));
    // //Figure out after there's a failsafe to not finding apriltags
    // driverController.x().onTrue(new SequentialCommandGroup(new ParallelCommandGroup(new
    // AdjustRobotPos(drivetrain, m_AprilTag),new
    // SetShooterAngle(m_Shooter,Constants.ShooterConstants.kShooterAutoAngle)),new
    // Shoot(m_ShooterRoller)));
    ...
    // // operatorController.leftBumper().and(operatorController.a()).whileTrue( new
    // OverrideIntakeDown(m_Intake));
    // // operatorController.leftBumper().and(operatorController.x()).whileTrue( new
    // OverrideIntakeUp(m_Intake));
